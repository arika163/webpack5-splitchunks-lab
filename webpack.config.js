...
module.exports = {
  ...
  optimization: {
    ...
    splitChunks: {
      ...
      // ❗ 注意：`enforceSizeThreshold` 是 webpack5 新增/常见的配置（用于强制按 maxSize/minSize 切分策略）。
      // 含义（在支持的版本中）：如果 chunk 的大小超过该阈值（字节），则会“强制”触发进一步分割，
      // 即使其他限制（比如 minChunks）没满足，也会进行切分以避免单个文件过大。
      // webpack4 中没有或不常用这个选项 —— 如果你用 webpack4，会报无效配置（或被忽略）。
